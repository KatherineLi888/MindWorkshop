...
function TierBlock({
  ...
  return (
    <div className="group relative w-full" style={{ height }}>
      ...
      <div
        className={cn(
          "pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-1 text-white",
          size === "main" && !compact ? "gap-5" : ""
        )}
      >
        {size === "main" ? (
          <div
            className={cn(
              "flex max-w-full items-baseline",
              compact ? "gap-1.5 sm:gap-2" : "gap-5"
            )}
          >
            <span
              className={cn(
                "shrink-0 font-semibold leading-none drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)]",
                compact ? "text-[9px] sm:text-[10px]" : "text-base"
              )}
            >
              {tier.label}
            </span>
            <span
              className={cn(
                "shrink-0 font-bold tabular-nums leading-none drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)]",
                compact ? "text-sm sm:text-base" : "text-3xl"
              )}
            >
              {tier.count}
            </span>
